...
manage_one_rule() {
    ...
    local check_regex="fwmark (${FWMARK_DEC}|${FWMARK_HEX}) lookup ${TABLE}"
    ...
    local rule_to_manage="from all fwmark ${FWMARK_DEC} lookup ${TABLE}"

    if [ "$ACTION" == "add" ]; then
        if [ "$rule_exists" == "no" ]; then
            echo "--> Adding ${ip_cmd} rule for fwmark ${FWMARK_DEC}"
            $ip_cmd rule add ${rule_to_manage}
        else
            echo "--> ${ip_cmd} rule for fwmark ${FWMARK_DEC} already exists, skipping."
        fi
    elif [ "$ACTION" == "del" ]; then
        if [ "$rule_exists" == "yes" ]; then
            echo "--> Deleting ${ip_cmd} rule for fwmark ${FWMARK_DEC}"
            $ip_cmd rule del ${rule_to_manage}
        else
            echo "--> ${ip_cmd} rule for fwmark ${FWMARK_DEC} does not exist, skipping."
        fi
    fi
}
